...
class TransactionTest_55 extends SimpleTest {
    ...
    $depth = DB::startTransaction();
      ...
      $depth = DB::startTransaction();
        ...
        $depth = DB::startTransaction();
          ...
          DB::query("UPDATE accounts SET `user.age`=%i WHERE username=%s", 500, 'Abe');
        $depth = DB::commit();
  ...
  function test_3_transaction_rollback_all() {
    ...
    DB::query("UPDATE accounts SET `user.age`=%i WHERE username=%s", 200, 'Abe');
    
    $depth = DB::startTransaction();
    $this->assert($depth === 1);
    DB::query("UPDATE accounts SET `user.age`=%i WHERE username=%s", 300, 'Abe');
    $depth = DB::startTransaction();
    $this->assert($depth === 2);
    
    DB::query("UPDATE accounts SET `user.age`=%i WHERE username=%s", 400, 'Abe');
